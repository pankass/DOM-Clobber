...
async function copyText(button, event, mode) {
    var text;
    ...
    const oldUserSelect = document.body.style.webkitUserSelect;
    ...
    document.body.style.webkitUserSelect = oldUserSelect;
    selection.removeAllRanges();
  }
    ...
    if (target == '')
        ...
        form.result.innerHTML =
            clobberer(target, customHrefValue && form.hrefValue.value)
                .map(html => `<div class="notification is-link is-light"><button class="copy-button" onclick="copyText(this,event,0)">copy</button><button class="copy-button" onclick="copyText(this,event,1)">copy mini</button><pre>${escapeHTML(html)}</pre></div>`).join('');
}